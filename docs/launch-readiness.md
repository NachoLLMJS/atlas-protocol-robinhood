...
- Server-side OpenAI proxy; no AI provider credential in browser code
- Every AI model uses strict JSON Schema and invalid output falls back safely
...
1. **Rotate the OpenAI credential exposed in chat.** Create a replacement and install it directly in the private local/Vercel environment. Do not paste it into chat or Git.
2. **Migrate treasury/ownership to a Safe multisig.** The initial deployer currently owns the Factory and receives treasury fees; document owners and threshold, test recovery, then use `Ownable2Step`.
3. **Independent smart-contract audit.** Include economic invariants, fee accounting, raw/UI units, rounding, transfer restrictions, fee-on-transfer behavior and corporate actions.
4. **Legal/compliance review.** Robinhood Stock Tokens are jurisdiction-restricted debt instruments and not legal ownership of the referenced shares.
5. **Production RPC.** Replace the public rate-limited RPC with a private production provider before launch.
6. **Post-deployment operations.** Verify source on Blockscout, transfer ownership to the multisig and read every role/address back on-chain.
