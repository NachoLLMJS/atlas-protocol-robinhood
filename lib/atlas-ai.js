...
const STRUCTURED_MODEL_IDS = new Set(['seed-2-0-lite-260428']);
...
function buildSystemPrompt(context) {
  ...
  return `You are ATLAS, the concise multilingual trading terminal for ATLAS Protocol on Robinhood Chain mainnet (chain ID 4663).
...
{"reply":"concise response in the user's language","action":"none|buy_index|sell_index|buy_stock|sell_stock|create_index|send|portfolio|price","data":{"ticker":"TSLA","indexTicker":"TECH","qty":1,"ethAmount":0.01,"indexName":"My Index","indexStocks":["TSLA","AMD"],"toAddress":"0x...","sendAmount":1,"sendToken":"TSLA"}}

Rules:
- Use action=buy_index only when a deployed indexTicker and qty are known; this means direct collateralized mint, not a free vault purchase.
- Use action=sell_index only when a deployed indexTicker and qty are known; this means burn and deterministic basket redemption.
...
- Respond in the user's language and keep replies short.`;
}
...
function readProviderError(payload, status) {
  ...
  if (status === 401 || status === 403 || /AccountOverdue|Authentication|Unauthorized/i.test(code)) {
...
function createAtlasAI({ fetchImpl = globalThis.fetch, env = process.env } = {}) {
  ...
  const baseUrl = (env.ARK_BASE_URL || 'https://ark.ap-southeast.bytepluses.com/api/v3').replace(/\/$/, '');
  const models = String(env.ATLAS_AI_MODELS || env.ARK_MODELS || '').split(',').map(model => model.trim()).filter(Boolean);
  ...
  return async function runAtlasAI(input) {
    ...
    for (const model of fallbackModels) {
      let response;
      try {
        const requestBody = { model, messages, temperature: 0.2, max_tokens: 800, stream: false };
        if (STRUCTURED_MODEL_IDS.has(model)) {
          requestBody.thinking = { type: 'disabled' };
          requestBody.response_format = ATLAS_RESPONSE_FORMAT;
        }
